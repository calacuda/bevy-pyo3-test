...
build-bin:
  ...
  cargo update
  # echo "build debug"
  # SYSROOT=$(pwd)/cross-build-deps/aarch64.3/untouched/ PKG_CONFIG_SYSROOT_DIR=${SYSROOT} PKG_CONFIG_LIBDIR=${SYSROOT}/usr/lib/pkgconfig:${SYSROOT}/usr/share/pkgconfig PKG_CONFIG_PATH=${SYSROOT}/usr/lib/pkgconfig/ PKG_CONFIG_ALLOW_CROSS=1 LD_LIBRARY_PATH=${SYSROOT}/lib/ cargo zigbuild --target aarch64-unknown-linux-gnu.2.33
  # echo "build release"
